...
namespace RLM
{
    ...
    public class RlmNetwork : IRlmNetwork
    {
        ...
        public IEnumerable<RlmSessionHistory> GetSessionHistory(int pageFrom = 0, int pageTo = 50)
        {
        ...
        public IEnumerable<RlmSessionHistory> GetSignificantLearningEvents(int pageFrom = 0, int pageTo = 50)
        {
            var resultCount = pageTo - pageFrom;
            IEnumerable<RlmSessionHistory> retVal = null;
            using (RlmDbEntities db = new RlmDbEntities(DatabaseName))
        ...
        public IEnumerable<RlmCaseHistory> GetSessionCaseHistory(long sessionId, int pageFrom = 0, int pageTo = 50)
        {
            ...
            using (RlmDbEntities db = new RlmDbEntities(DatabaseName))
            {
                string query = $"SELECT Id, CycleStartTime AS DateTimeStart, CycleEndTime AS DateTimeStop, CycleScore, Session_Id AS SessionId, Rneuron_Id AS RneuronId, Solution_Id AS SolutionId FROM Cases where Session_ID = {sessionId};";
