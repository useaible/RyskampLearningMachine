...
namespace RetailPoC.Models
{
    public class Shelf
    {
        ...
        {            
            ...
            items.Add(new ShelfItem() { SKU = item.SKU, ItemID = item.ID, Index = Convert.ToInt32(item.SKU), Name = item.Name, Score = metricScore, Order = items.Count + 1, DrawingColor = dcolor, Color = System.Windows.Media.Color.FromArgb(dcolor.A, dcolor.R, dcolor.G, dcolor.B) });
        }
